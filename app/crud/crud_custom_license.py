...
class CRUDCustomLicense(CRUDBase[CustomLicense, CustomLicenseCreate, CustomLicenseUpdate]):
    def get_by_license_id(
        self, db: Session, *, license_id: str
    ) -> Optional[CustomLicense]:
        return db.query(CustomLicense).filter(CustomLicense.license_id == license_id).first()
